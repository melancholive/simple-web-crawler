...
for result in search_results[:10]: 
    ...
    if i != -1: # make sure that the marker exists
        ...
        site_queue.put((-2, 0, final_url, parse_url(final_url))) # priority, depth, url
...
# log to log.txt and /webpages directory
...
while not site_queue.empty() and num_visited < max_download:
    ...
    try:
        ...
        # 404 : the web server reached out successfully
        # but it could not find the specific page or file you requested

        # 403 : FORBIDDEN the web server understands your request
        # but refuses to grant access to the page or file

        # 302 : infinite loop from redirects
